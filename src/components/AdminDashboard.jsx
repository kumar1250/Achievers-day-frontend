      ...
      {stats.placement_stats && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            <StatCard
              label="Total Placed"
              value={stats.placement_stats.total_placed}
              theme="slate"
              delay={0}
            />
            <StatCard
              label="IT Roles"
              value={stats.placement_stats.it_count}
              theme="blue"
              delay={60}
            />
            <StatCard
              label="Non-IT Roles"
              value={stats.placement_stats.non_it_count}
              theme="amber"
              delay={120}
            />
            <StatCard
              label="Companies"
              value={stats.placement_stats.unique_companies}
              theme="violet"
              delay={180}
            />
            <StatCard
              label="Avg Salary (LPA)"
              value={stats.placement_stats.average_salary ?? 0}
              theme="emerald"
              delay={240}
            />
          </div>

          {stats.placement_stats.highest_salary != null && (
            <p className="text-xs text-slate-500 mt-2">
              Highest package recorded:{" "}
              <span className="font-bold text-emerald-700">
                {stats.placement_stats.highest_salary} LPA
              </span>
            </p>
          )}

          {/* Branch-wise placed count bars */}
          {stats.placement_stats.branch_wise?.length > 0 && (
            <div className="mt-4 bg-white rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-5">
              ...
              <div className="space-y-2">
                {(() => {
                  const max = Math.max(
                    ...stats.placement_stats.branch_wise.map((b) => b.count)
                  );
                  return stats.placement_stats.branch_wise.map((b, i) => (
                    <div key={b.branch} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-3 rounded-full bg-slate-100 overflow-hidden">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-violet-500 to-blue-600 animate-card-in"
                          style={{
                      ...
                      <span className="w-8 shrink-0 text-xs font-bold text-slate-700 text-right">
                        {b.count}
                      </span>
                    </div>
                  ));
        ...
        <div className="px-5 py-3 border-b border-slate-200 bg-gradient-to-r from-violet-50 via-blue-50 to-emerald-50 flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[160px] max-w-xs">
            ...
            <input
              ...
              placeholder="Search name or regd no..."
