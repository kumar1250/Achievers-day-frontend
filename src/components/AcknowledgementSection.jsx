...
export default function AcknowledgementSection({ student, onRegisterAnother }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.setProperties({
        title: `Achievers Day Acknowledgement - ${student.roll_no}`,
      });
      pdf.addImage(imgData, "PNG", x, y, imgWidth, imgHeight);

      const filename = `Achievers_Day_Acknowledgement_${student.roll_no}.pdf`;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 font-['Sora']">
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 bg-[#00B8A9]/10 border border-[#00B8A9]/30 text-[#5CE0D3] text-[11px] sm:text-xs font-semibold uppercase tracking-wide px-3 sm:px-4 py-1.5 rounded-full">
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
        ...
        <h2 className="font-['Fraunces'] text-xl sm:text-2xl md:text-3xl font-bold text-white mt-3">
          You're all set for Achievers Day
        </h2>
      ...
      <div
        ref={wrapperRef}
        className="mx-auto rounded-xl shadow-[0_24px_60px_-24px_rgba(0,0,0,0.6)] overflow-hidden"
      ...
      {error && (
        <div className="mt-4 max-w-lg mx-auto bg-[#FF5A5F]/10 border border-[#FF5A5F]/30 text-[#FF8A8D] text-sm rounded-xl px-4 py-3 text-center">
          {error}
        </div>
      )}
      ...
      <div className="flex flex-wrap justify-center gap-3 mt-6">
        <button
          ...
          className="inline-flex items-center gap-2 bg-[#F4A100] hover:bg-[#FFC94A] text-[#14102B] font-bold px-5 sm:px-7 py-3 rounded-xl shadow-[0_10px_30px_-8px_rgba(244,161,0,0.5)] disabled:opacity-50 transition text-sm sm:text-base"
        >
          <svg
            width="17"
            height="17"
        ...
        <button
          ...
          className="text-white/55 hover:text-white text-sm font-medium px-4 py-3 transition"
        >
